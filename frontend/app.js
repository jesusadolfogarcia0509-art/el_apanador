document.addEventListener('DOMContentLoaded', () => {
    ...
    const catButtons = Array.from(document.querySelectorAll('.cat-btn')); // Convertimos a Array para el índice
    
    let allSolutions = [];
    let currentCategory = 'all';
    let currentCatIndex = 0; // Para saber dónde estamos al deslizar
    ...
    function handleSwipe(startX, startY, endX, endY) {
        ...
        // Si el movimiento es horizontal y lo suficientemente largo (> 50px)
    ...
    function changeCategory(index) {
        // Comprobar límites
        if (index < 0 || index >= catButtons.length) return;

        // Actualizar índice
        currentCatIndex = index;
        const btn = catButtons[index];

        // Simular clic en el botón para activar todo
        btn.click();

        // Scroll automático del botón para que siempre esté visible
    ...
    // ACTUALIZACIÓN DEL EVENTO CLICK EN BOTONES DE CATEGORÍA
    catButtons.forEach((btn, index) => {
        btn.addEventListener('click', () => {
            // Actualizar índices y estilos
            currentCatIndex = index;
    ...
    function renderSolutions(solutions) {
        ...
        // MENSAJE PARA LISTA VACÍA (FAVORITOS O BÚSQUEDA)
