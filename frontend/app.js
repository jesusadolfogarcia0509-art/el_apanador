document.addEventListener('DOMContentLoaded', () => {
    ...
    const cameraInput = document.getElementById('cameraInput'); // Input invisible
    ...
    // ---------------------------------------------------------
    // ⚠️ ZONA DE CONFIGURACIÓN IA
    // ---------------------------------------------------------
    const GEMINI_API_KEY = "AIzaSyCkndPElYFtEXNMR1RJFGrdGt84RR114-A
"; 
    // ---------------------------------------------------------
    ...
    cameraInput.addEventListener('change', async (e) => {
        ...
        modalBody.innerHTML = `
            <div class="ai-loading" style="text-align:center; padding:40px; color:white;">
                ...
                <h3 style="margin-top:20px;">Analizando tu foto con IA...</h3>
                <p style="color:#cbd5e1; font-size:0.9rem;">Esto puede tardar unos segundos.</p>
            </div>
        `;
        ...
        try {
            ...
            // Abrir modal con el resultado de la IA
            openModal({
                title: result.title || "Objeto Detectado",
                solution_text: result.text || "Aquí tienes información sobre esta herramienta.",
                image_url: URL.createObjectURL(file), // Mostramos la foto que subió el usuario
                category: "IA Detectada",
                risk_level: "Bajo",
                difficulty: 1,
                // Búsquedas automáticas basadas en lo que detectó
        ...
        // Limpiar input para poder subir la misma foto si quiere
        cameraInput.value = '';
    });
    ...
    async function analyzeWithGemini(base64Image) {
        ...
        const prompt = "Analiza esta imagen. Identifica qué herramienta u objeto de bricolaje/hogar es. Responde SOLO con un JSON válido (sin markdown ```json) con estos campos: { \"title\": \"Nombre corto del objeto\", \"keyword\": \"Palabra clave para comprarlo\", \"text\": \"Explica en 2 frases para qué sirve y un consejo rápido de uso.\" }";
        ...
        // Limpiar formato por si la IA devuelve ```json ... ```
